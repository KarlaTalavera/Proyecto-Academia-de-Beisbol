...
router.get('/taquilla', verificarToken, soloRoles('administrador', 'caja'), async (req, res) => {
  ...
  const [rows] = await db.query(
    `SELECT
       ...
       COALESCE(SUM(CASE WHEN LOWER(i.concepto) LIKE '%vip%'     THEN i.valor ELSE 0 END), 0) AS recaudado_vip,
       COALESCE(SUM(CASE WHEN LOWER(i.concepto) NOT LIKE '%vip%'
                          AND (LOWER(i.concepto) LIKE '%boleto%'
                            OR LOWER(i.concepto) LIKE '%taquill%'
                            OR LOWER(i.concepto) LIKE '%entrada%')
                         THEN i.valor ELSE 0 END), 0)                                          AS recaudado_general,
       COALESCE(SUM(CASE WHEN LOWER(i.concepto) LIKE '%boleto%'
                          OR LOWER(i.concepto) LIKE '%taquill%'
                          OR LOWER(i.concepto) LIKE '%entrada%'
                          OR LOWER(i.concepto) LIKE '%vip%'
                         THEN i.valor ELSE 0 END), 0)                                          AS recaudado_total
     FROM partido p
     JOIN equipo ec ON p.id_equipo_casa      = ec.id_equipo
     JOIN equipo ev ON p.id_equipo_visitante  = ev.id_equipo
     LEFT JOIN resultado r ON r.id_partido = p.id_partido
     LEFT JOIN ingreso i   ON i.id_temporada = p.id_temporada
       AND (LOWER(i.concepto) LIKE '%boleto%'
         OR LOWER(i.concepto) LIKE '%taquill%'
         OR LOWER(i.concepto) LIKE '%entrada%'
         OR LOWER(i.concepto) LIKE '%vip%')
     WHERE p.id_temporada = ? AND p.estado = 'finalizado'
     GROUP BY p.id_partido, p.fecha_juego, p.lugar,
              ec.nombre_equipo, ev.nombre_equipo,
...
// ── Estadísticas ofensivas de bateadores ─────────────────
router.get('/estadisticas-bateadores', verificarToken, async (req, res) => {
  ...
  const [rows] = await db.query(
    `SELECT
       ...
       SUM(db.turnos_al_bate)             AS AB,
       SUM(db.hits)                       AS H,
       SUM(db.dobles)                     AS dobles,
       SUM(db.triples)                    AS triples,
       SUM(db.jonrones)                   AS HR,
       SUM(db.carreras_impulsadas)        AS RBI,
       SUM(db.carreras)                   AS R,
       COUNT(DISTINCT db.id_partido)      AS juegos,
       ROUND(SUM(db.hits) / NULLIF(SUM(db.turnos_al_bate), 0), 3) AS AVE
     FROM desempeno_bateador db
     JOIN jugador j ON db.id_jugador = j.id_jugador
     JOIN equipo  e ON db.id_equipo  = e.id_equipo
     JOIN partido p ON db.id_partido = p.id_partido
     WHERE p.id_temporada = ?
     GROUP BY j.id_jugador, j.nombre, j.apellido, e.nombre_equipo
     HAVING AB > 0
     ORDER BY AVE DESC`,
