...
const AuthController = {
  async login(req, res) {
    ...
    const token = jwt.sign(
      { id: usuario.id_usuario, rol: usuario.rol, nombre: usuario.nombre, id_equipo: usuario.id_equipo },
      process.env.JWT_SECRET,
      { expiresIn: process.env.JWT_EXPIRES_IN || '8h' }
    )
    res.json({ token, rol: usuario.rol, nombre: usuario.nombre, id_equipo: usuario.id_equipo })
  },
